...
export class WSocket {
  ...
  constructor(url: string) {
    ...
    this.socket = new WebSocket(url); // wss//steps-u8bq.onrender.com
    ...
    this.socket.addEventListener("close", (e: any) => {
      if (e.wasClean === true) console.warn("[WebSocket EventListenerClose]:", this.socket.readyState);
      else {
        this.onError(e);
      }
    });
  ...
  set onMessage(e: Record<string, any>) {
    ...
    localStorage.setItem("heandlersData", e["data"] as string);
  ...
  onError(e: any): void {
    console.log("[WebSocked onError]: has been break", e);
  }
  /**
  ...
  set onSend(prop: string) {
    const transactionKeys = ["open", "close", "insert", "data", "delete", "edit"];
  ...
  get onSend(): string {
    ...
    const stepsStr = JSON.stringify({ ...this.heandlers });
    this.socket.send(stepsStr);
    return JSON.stringify(stepsStr);
  }
